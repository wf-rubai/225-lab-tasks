...
template <class VertexType>
bool GraphType<VertexType>::isQueued(VertexType v, quetype<VertexType>& q)
{
    VertexType c;
    bool found = false;
    quetype<VertexType> qq;
    while(!q.IsEmpty()){
        q.Dequeue(c);
        if(c == v){
            found = true;
...
void GraphType<VertexType>::BreadthFirstSearch(VertexType startVertex, VertexType endVertex)
{
    ...
    quetype<VertexType> vertexQ;
    bool found = false;
    VertexType vertex = startVertex, item;
    int length = 0;
    ClearMarks();
    queue.Enqueue(startVertex);
    do
    {
        if(!isQueued(vertex, queue)){
            length++;
        }
...
{
    int n = IndexIs(vertices, u);
    int m = IndexIs(vertices, v);
    return ((edges[n][m] == 1) || (edges[m][n] == 1));
}
